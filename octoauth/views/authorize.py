...
from octoauth.domain.oauth2.services import ApplicationService, AuthorizationService, ScopeService, TokenService
...
def display_authorization_form(
    ...
    show_consent_dialog: bool = Query(False, description="Boolean value that indicates whether consent dialog should be displayed even if permission has already been granted.")
):
    ...
    if not show_consent_dialog:
        ...
        already_granted_scopes = ScopeService.get_client_granted_scopes(account_dto.uid, authorization_params.client_id)
...
def submit_authorization_form(
    ...
    if authorization_params.response_type == ResponseType.CODE:
        ...
        response = RedirectResponse(authorization_params.redirect_uri + "#" + urlencode(response_data), status.HTTP_303_SEE_OTHER)
        ...
        return RedirectResponse(authorization_params.redirect_uri + "?" + urlencode(grant_data.dict()), status.HTTP_303_SEE_OTHER)
    else:
